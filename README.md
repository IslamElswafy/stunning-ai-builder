...
| AI | OpenRouter chat-completions API (OpenAI-compatible), default model `openrouter/free` |
...
| --- | --- | --- |
| `OPENROUTER_API_KEY` | Yes | OpenRouter API key ([openrouter.ai/keys](https://openrouter.ai/keys)). Read only inside `lib/ai.ts`, which runs on the server, and sent as `Authorization: Bearer`. |
| `AI_MODEL` | No | Overrides the model. Defaults to `openrouter/free`. |

The app uses the provider's own variable name rather than a generic `AI_API_KEY`: one clearly-named variable beats an indirection layer around a single call site. `openrouter/free` routes across zero-cost models, so a different one may serve each request.
...
- **`lib/ai.ts`** — the only file that talks to the provider. Everything above it deals in "system prompt in, text chunks out".
